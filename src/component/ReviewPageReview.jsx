...
export default function ReviewComponent() {
  ...
  return (
    <div className={styles.container}>
      {data
        .filter((review) =>
          selectedGenre === "범죄" ? review.notice_genre === "범죄" : true
        )
        .filter((review) =>
          selectedGenre === "코미디" ? review.notice_genre === "코미디" : true
        )
        .filter((review) =>
          selectedGenre === "드라마" ? review.notice_genre === "드라마" : true
        )
        .filter((review) =>
          selectedGenre === "모험" ? review.notice_genre === "모험" : true
        )
        .filter((review) =>
          selectedGenre === "키즈" ? review.notice_genre === "키즈" : true
        )
        .filter((review) =>
          selectedGenre === "액션" ? review.notice_genre === "액션" : true
        )
        .filter((review) =>
          selectedGenre === "판타지" ? review.notice_genre === "판타지" : true
        )
        .filter((review) =>
          selectedGenre === "애니메이션"
            ? review.notice_genre === "애니메이션"
            : true
        )
        .filter((review) =>
          selectedGenre === "스릴러" ? review.notice_genre === "스릴러" : true
        )
        .filter((review) =>
          selectdrating === "5" ? review.rating === 5 : true
        )
        .filter((review) =>
          selectdrating === "4.5" ? review.rating === 4.5 : true
        )
        .filter((review) =>
          selectdrating === "4" ? review.rating === 4 : true
        )
        .filter((review) =>
          selectdrating === "3.5" ? review.rating === 3.5 : true
        )
        .filter((review) =>
          selectdrating === "3" ? review.rating === 3 : true
        )
        .filter((review) =>
          selectdrating === "2.5" ? review.rating === 2.5 : true
        )
        .filter((review) =>
          selectdrating === "2" ? review.rating === 2 : true
        )
        .filter((review) =>
          selectdrating === "1.5" ? review.rating === 1.5 : true
        )
        .filter((review) =>
          selectdrating === "1" ? review.rating === 1 : true
        )
        .filter((review) =>
          selectdrating === "0.5" ? review.rating === 0.5 : true
        )
              ...
              <div className={styles.reviewTitleContainer}>
                <button className={styles.rating__starBtn}>
                  <div className={styles["rating__starBtn__elements-group"]}>
                    ...
                    <span className={styles.rating__starBtn__text}>
                      {review.rating}
                    </span>
                  </div>
                </button>
